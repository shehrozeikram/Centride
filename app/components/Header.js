...
const Header = ({
  ...
  return (
    <View style={[Style.flexRow, Style.justifyBetween, Style.alignCenter]}>
      ...
      {isMenuIcon && (
        <TouchableOpacity onPress={handleMenu} disabled={loading}>
          {loading ? (
      ...
      {isDriver ? (
        <DriverDrawer isVisible={isModalVisible} onClose={toggleDrawer} />
      ) : (
        <DrawerModal isVisible={isModalVisible} onClose={toggleDrawer} />
      )}
    </View>
  );
};
