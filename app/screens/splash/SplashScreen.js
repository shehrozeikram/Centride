...
const SplashScreen = () => {
  ...
  const handleRoute = async () => {
    try {
      ...
      if (!phone) {
        Alert.alert("Please Login");
        console.log("No phone found, redirecting to RiderStackNavigator");
        navigation.reset({
          index: 0,
          routes: [{ name: "RiderStackNavigator" }],
        });
      ...
      if (!isLoggedIn) {
        console.log("User not logged in, redirecting to RiderStackNavigator");
        navigation.reset({
          index: 0,
          routes: [{ name: "RiderStackNavigator" }],
        });
    ...
    if (password) {
      ...
      try {
        ...
        if (data.loggedin === 1) {
          ...
          const ongoingBkData = data?.ongoing_bk ? data.ongoing_bk : {};
          ...
          navigation.reset({
            ...
            routes: [
              {
                name: "RiderRoute",
                params: { ongoing_bk: ongoingBkData },
              },
            ],
          });
        } else {
  ...
  const handleLoginDriver = async (password, phone) => {
    ...
    if (password) {
      try {
        ...
        const phoneFormatted = phone.startsWith("0") ? phone.slice(1) : phone;
