...
const DropOffLocation = ({ navigation }) => {
  ...
  // Debounce the setSearchQuery2 function
  const debouncedSetSearchQuery = useCallback(
    debounce((text) => {
      setSearchQuery2(text);
    }, 400), // Adjust debounce delay (e.g., 300ms)
    []
  );
  ...
  const handleRoutes = () => {
    if (route?.params) {
      ...
      if (route?.params?.origin?.latitude && route?.params?.origin?.longitude) {
        reverseGeocode(
  ...
  return (
    <View style={Style.container}>
      ...
      <View style={styles.searchBarContainer}>
        <GooglePlacesAutocomplete
          ...
          textInputProps={{
            ...
            value: searchQuery || userAddress || routeValue?.address || "", // Conditionally set value
          }}
          renderRow={(data) => (
            <View style={styles.suggestionContainer}>
              <FontAwesome name="map-marker" size={22} color="red" />
      ...
      {searchQuery2 === "" && searchQuery3 === "" && searchQuery === "" ? (
        <>
          <Text
            style={[
